...
def convert_pred(y_pred, boxsx, boxsy, anchors, tl_cell):
...
def pos_true_ws(n_bat, n_anchors, boxsx, boxsy, anchors, tl_cell):
    size4calc = [boxsx, boxsy]
    anchors_whole_image = tf.divide(anchors, size4calc)
    # rowz = np.divide(np.arange(boxsy, dtype=np.float32), boxsy)
    cent_true = tf.fill((n_bat, boxsy, boxsx, n_anchors, 2), 0.5)
...
def calc_size_cent(size_cnn_raw, cent_cnn_raw, size_true, cent_true, boxsx, boxsy, anchors, tl_cell):
    # set base values
    size4calc = [boxsx, boxsy]
...
def loss_gfrc_yolo(gt1, gt2, y_pred, dict_in):
    ...
    confs_cnn, cent_cnn, size_cnn, class_cnn = convert_pred(y_pred, boxsx, boxsy, anchors, tl_cell)
    ...
    noonesij = tf.cast(tf.less(best_iou, thresh), tf.float32)
    onesij = confs_true
    ...
    ctl_x, ctl_y, szl_x, szl_y = \
        calc_size_cent(size_cnn_raw, cent_cnn_raw, size_true, cent_true, boxsx, boxsy, anchors, tl_cell)
...
def loss_gfrc_yolo_ws(gt1, gt2, y_pred, dict_in):
    ...
    conf_loss_nogt = 0.5
    conf_loss_gt = 0.5
    test1 = tf.reduce_sum(confs_cnn)

    # set detection positions at centre of cell of size anchor box
    cent_true, size_true = pos_true_ws(n_bat, nanchors, boxsx, boxsy, anchors, tl_cell)
