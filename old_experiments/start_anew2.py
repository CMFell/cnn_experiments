...
BATCH_SIZE = 20
WARM_UP_EXAMPLES = 10000
WARM_UP_BATCHES = int(WARM_UP_EXAMPLES / BATCH_SIZE)
TRUE_BOX_BUFFER = 15

wt_path = "E:/CF_Calcs/BenchmarkSets/GFRC/ToUse/Train/yolo-gfrc_6600.weights"
train_image_folder = 'E:/CF_Calcs/BenchmarkSets/GFRC/yolo_train384_5pc_img/'
train_annot_folder = 'E:/CF_Calcs/BenchmarkSets/GFRC/yolo_train384_5pc_ann_voc/'
valid_image_folder = 'E:/CF_Calcs/BenchmarkSets/GFRC/yolo_valid384_subset_img/'
valid_annot_folder = 'E:/CF_Calcs/BenchmarkSets/GFRC/yolo_valid384_subset_ann_voc/'
valid_full_image_folder = 'E:/CF_Calcs/BenchmarkSets/GFRC/yolo_valid384_img/'
valid_full_annot_folder = 'E:/CF_Calcs/BenchmarkSets/GFRC/yolo_valid384_ann_voc/'
...
for i in range(1, nb_conv + 1):
    ...
    if len(conv_layer.get_weights()) > 1:
        print("conv size", np.prod(conv_layer.get_weights()[0].shape), np.prod(conv_layer.get_weights()[1].shape))
        ...
        print("conv size", np.prod(conv_layer.get_weights()[0].shape))
        kernel = weight_reader.read_bytes(np.prod(conv_layer.get_weights()[0].shape))
        kernel = kernel.reshape(list(reversed(conv_layer.get_weights()[0].shape)))
        kernel = kernel.transpose([2, 3, 1, 0])
...
def step_decay(epoch):
    ...
    drop = 0.1
    epochs_drop = 10.0
    lnrate = init_lrate * math.pow(drop, math.floor((epoch + 1) / epochs_drop))
    return lnrate
