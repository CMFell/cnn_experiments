...
def create_tiled_data(filez, fl, base_dir, data_list, out_path, windowz, image_shape):
    file_name = filez[fl]
...
# folder to save in
out_folder = 'CF_Calcs/BenchmarkSets/GFRC/yolo_test_lg/'
...
gfrcrowst = np.linspace(0, input_shape[0], n_split[0] + 1)[:-1]
print(gfrcrowst)
gfrcrowst = np.reshape(np.tile(gfrcrowst, n_split[0]), (np.square(n_split[0]), 1))
gfrcrowed = np.linspace(0, input_shape[0], n_split[0] + 1)[1:]
gfrcrowed = np.reshape(np.tile(gfrcrowed, n_split[0]), (np.square(n_split[0]), 1))
gfrccolst = np.linspace(0, input_shape[1], n_split[1] + 1)[:-1]
print(gfrccolst)
gfrccolst = np.reshape(np.repeat(gfrccolst, n_split[1]), (np.square(n_split[1]), 1))
gfrccoled = np.linspace(0, input_shape[1], n_split[1] + 1)[1:]
gfrccoled = np.reshape(np.repeat(gfrccoled, n_split[1]), (np.square(n_split[1]), 1))
gfrcwindz = np.hstack((gfrcrowst, gfrccolst, gfrcrowed, gfrccoled))
gfrcwindz = np.array(gfrcwindz, dtype=np.int)
print(gfrcwindz.shape)
print(gfrcwindz[0,:])
...
for ff in range(filez_in.shape[0]):
    create_tiled_data(filez_in, ff, basedir, datalist, outpath, gfrcwindz, gfrcsize)
